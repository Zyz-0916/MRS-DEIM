...
from engine.extre_module.custom_nn.block.rgelan import RGELAN

from engine.extre_module.custom_nn.neck.mrfpan import MRFPAN, FocusFeature, ADown, SmallObjectEnhancer  
    ...
    def __init__(self, \
        ...
        if pretrained:
            ...
            try:
                ...
                # need_keep_key_prefix_list = ['0.*', '1.*','2.*']
                # need_pop_key_list = []  
...
def parse_module(d, i, f, m, args, ch, nc=None, eval_spatial_size=None):   
    ...
    if m in {StemBlock, HG_Stage}:    
    ...
    elif m is SmallObjectEnhancer: 
        c2 = ch[f] 
        
        if len(args) >= 2 and args[0] == c2:
        ...
        c1, c2 = ch[f], args[0]     
        args = [c1, c2, *args[1:]]  
    elif m in {C3_Block, C2f_Block, C3k2_Block, MetaFormer_Block, MetaFormer_Mona, MetaFormer_SEFN, MetaFormer_Mona_SEFN, RGELAN}: # Block  
        c1, c2 = ch[f], args[0]
