...
from engine.extre_module.custom_nn.conv_module.SFSConv import ChannelAttention, SpatialAttention
   
__all__ = ['MRFPAN', 'SmallObjectEnhancer']
...
class SmallObjectEnhancer(nn.Module):
    ...
    def __init__(self, channels, reduction=16, kernel_size=7):
        ...
        # 1. 多尺度小核卷积分支（避免大核损失细节）
        self.branch1 = nn.Sequential(
            Conv(channels, channels, 3, 1, 1),  # 3x3标准卷积
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(channels, channels, 5, 1, 2, groups=channels),
            nn.BatchNorm2d(channels),
            nn.SiLU(),
            nn.Conv2d(channels, channels, 1, 1, 0),  # 1x1逐点卷积
            nn.BatchNorm2d(channels),
            nn.SiLU()
        )
    ...
    def forward(self, x):
        ...
        feat1 = feat1 * ca_weight
        feat2 = feat2 * ca_weight
        ...
        # 空间注意力（增强边缘）
        sa_weight = self.spatial_attention(fused)
        fused = fused * sa_weight
        
        # 自适应残差连接
        out = fused + identity * self.residual_weight
...
class MRFPAN(nn.Module):
    def __init__(self, 
                 ...
                 fdpn_ks=[3, 5, 7, 9],                 # MRFPAN中的FocusFeature-kernel_sizes参数
...
if __name__ == '__main__': 
    ...
    module = MRFPAN(**params).to(device) 
